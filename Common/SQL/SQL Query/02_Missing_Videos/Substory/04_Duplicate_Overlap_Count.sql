-- ============================================================
-- MISSING VIDEOS — Substory Duplicate Overlap Count
-- Purpose : Count duplicates and overlaps in one query
-- Table   : sub_story
...
WITH base AS (
    ...
    FROM sub_story s
    WHERE start_time >= '2026-02-24 05:00:00'    -- ⬅ Change Start Time
...
dups AS (
    SELECT channel_code, start_time, end_time
    FROM base
...
SELECT
    (SELECT COUNT(*) FROM dups)     AS duplicate_count,
    (SELECT COUNT(*) FROM overlaps) AS overlap_count;
